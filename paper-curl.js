      ...
      const job = Promise.resolve().then(async () => {
        ...
        try {
          ...
          const styles = nodes.map((node) => {
            ...
            // Resolve inherited strokes and CSS variables while the clone is
            // still in the document; its stylesheet rules will not travel with it.
            const visualProperties =
              node.namespaceURI === "http://www.w3.org/2000/svg"
                ? properties.concat(svgProperties)
                : properties;
            return visualProperties.map((property) => [
              property,
